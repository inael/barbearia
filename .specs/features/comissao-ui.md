...
|-------|------------------------|-----------|-----------|--------|----------|
| CUI-001 | `/comissao` responde 200 e mostra título "Comissao & Pote" | e2e | e2e/comissao.spec.ts | PENDING | falta |
| CUI-002 | Faturamento mês anterior 8000→"40%", 13000→"45%", 15000→"50%" (faixa de serviço exibida) | e2e | e2e/comissao.spec.ts | PENDING | falta |
| CUI-003 | Total do barbeiro = avulsos×faixa + combos×40% + divididos×20% + produto×faixaProduto (bate com o motor) | e2e | e2e/comissao.spec.ts | PENDING | falta |
| CUI-004 | Pote total exibido = 40% da receita de assinaturas; soma da divisão por pontos = pote | e2e | e2e/comissao.spec.ts | PENDING | falta |
| CUI-005 | Total da recepção = divididos×20% + comissão de hidratações (R$5/un, R$10/un se >10) | e2e | e2e/comissao.spec.ts | PENDING | falta |
| CUI-006 | Consistência UI↔motor: para uma entrada fixada, os valores exibidos == `lib/comissao`+`lib/pote` computados diretamente | e2e | e2e/comissao.spec.ts | PENDING | falta |
