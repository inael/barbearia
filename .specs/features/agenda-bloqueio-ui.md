...
|-------|------------------------|-----------|-----------|--------|----------|
| BLQUI-001 | `criarBloqueio` insere e o bloqueio aparece em `listarBloqueios` (com motivo) | integration | lib/db/agenda.integration.test.ts | PENDING | falta |
| BLQUI-002 | `criarBloqueio` com `inicio >= fim` lança | integration | lib/db/agenda.integration.test.ts | PENDING | falta |
| BLQUI-003 | `removerBloqueio` NÃO apaga bloqueio de outro barbeiro (segurança) | integration | lib/db/agenda.integration.test.ts | PENDING | falta |
| BLQUI-004 | `removerBloqueio` do próprio remove | integration | lib/db/agenda.integration.test.ts | PENDING | falta |
| BLQUI-005 | `/minha-agenda/bloqueios` sem login redireciona pra `/login` (proxy) | e2e | e2e/agenda-bloqueios.spec.ts | PENDING | falta |
| BLQUI-006 | barbeiro cria um bloqueio (aparece na lista) e remove (some) | e2e | e2e/agenda-bloqueios.spec.ts | PENDING | falta |
